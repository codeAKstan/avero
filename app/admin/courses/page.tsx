...
export default function CourseManagementPage() {
  ...
  const toggleRandomize = async (course: any) => {
    try {
      const res = await fetch(`/api/admin/courses/${course._id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ randomizeQuestions: !course.randomizeQuestions }),
      });
      if (res.ok) fetchCourses();
    } catch (err) {
      console.error(err);
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          ...
          <div className="relative">
            <button
              ...
              className="px-4 py-2.5 bg-white hover:bg-slate-50 text-slate-700 border border-slate-200 text-xs font-bold rounded-xl flex items-center gap-2 shadow-xs transition cursor-pointer"
            >
            ...
            {isExportMenuOpen && (
              <>
                ...
                />
                <div className="absolute right-0 mt-2 w-64 bg-white border border-slate-200 rounded-xl shadow-xl z-30 p-1.5 text-xs space-y-1 animate-in fade-in zoom-in-95 duration-100">
                  <button
                    type="button"
                    onClick={() => {
                      exportCoursesSummaryCSV(courses);
                  ...
                  >
                    ...
                    <div>
                      <div className="font-bold text-slate-900">Course Summary (CSV)</div>
                      <div className="text-[10px] text-slate-500">Titles, categories, question counts & status</div>
                    </div>
                  </button>
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      exportQuestionBankCSV(courses);
                      setIsExportMenuOpen(false);
                    }}
                    className="w-full text-left px-3 py-2.5 hover:bg-slate-50 rounded-lg font-medium text-slate-700 flex items-center gap-2.5 cursor-pointer transition"
                  >
                    <FileSpreadsheet className="w-4 h-4 text-[#2866e1] shrink-0" />
                    <div>
                      <div className="font-bold text-slate-900">Full Question Banks (CSV)</div>
                      <div className="text-[10px] text-slate-500">Questions, choices A-D & verbatim rationales</div>
                    </div>
                  </button>
                  ...
                  >
                    ...
                    <div>
                      <div className="font-bold text-slate-900">Full Course Package (JSON)</div>
                      <div className="text-[10px] text-slate-500">Structured JSON backup with modules & questions</div>
                    </div>
                  </button>
                </div>
              </>
